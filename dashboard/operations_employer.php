...
#pagination	
	...
	$orderCountQuery = $pdo->prepare("SELECT COUNT(*) as count FROM orders WHERE employer_id=:id");
	$orderCountQuery->execute(['id' => $employerId]);
	...
	$ordersQuery->execute(['id' => $employerId]);
							...
							<table class="basic-table">
								...
								<?php while($order = $ordersQuery->fetch(PDO::FETCH_ASSOC)){ ?>
									<tr>
										...
										<td data-label="Sil"><?php if($order['completed_status'] == 0){?><a href="../system/operationEmployerController.php?operation=delete&id=<?php echo $order['order_id'];?>" style="color:black;font-size:20px;"><i class="icon-feather-delete"></i></a><?php }?></td>	
									</tr>
								<?php }?>
							</table>
						</div>
