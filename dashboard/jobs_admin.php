...
#pagination	
	...
	$jobCountQuery = $pdo->prepare("SELECT COUNT(*) as count FROM jobs");
							...
							<table class="basic-table">
								...
								<?php while($job = $jobsQuery->fetch(PDO::FETCH_ASSOC)){ ?>
									<tr>
										...
										<td data-label="Frilanser Ad/Soyad"><?php echo $job['first_name'].' '.$job['last_name']; ?></td>
										...
										<td data-label="Düzəliş et"><a href="edit_job_admin.php?id=<?php echo $job['job_id'];?>" style="color:black; font-size:20px;"><i class="icon-feather-edit"></i></a></td>
										<td data-label="İşə bax"><a href="view_job_admin.php?&id=<?php echo $job['job_id'];?>" style="color:black;font-size:20px;"><i class="icon-feather-search"></a></i></td>
										<td data-label="Sil"><a href="../system/jobAdminController.php?operation=delete&id=<?php echo $job['job_id'];?>" style="color:black;font-size:20px;"><i class="icon-feather-delete"></a></i></td>
										<?php
											if($job['verification_status']){
										?>
											<td data-label="Aktiv/Deaktiv et"><a href="../system/jobAdminController.php?operation=toggle&id=<?php echo $job['job_id'];?>" class="button" style="background-color:red;color:white;font-size:13px;">Deaktiv et</a></td>
										<?php } else{?>	
											<td data-label="Aktiv/Deaktiv et"><a href="../system/jobAdminController.php?operation=toggle&id=<?php echo $job['job_id'];?>" class="button" style="background-color:green;color:white;">Aktiv et</a></td>
										<?php } ?>
									</tr>
								<?php }?>
							</table>
