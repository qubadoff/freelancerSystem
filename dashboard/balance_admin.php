...
#pagination	
	...
	$transactionCountQuery = $pdo->prepare("SELECT COUNT(*) as count FROM transactions");
	$transactionCountQuery->execute();
	$transactionCount = $transactionCountQuery->fetch(PDO::FETCH_ASSOC);

	$totalCommissionQuery = $pdo->prepare("SELECT SUM(amount) as total_commission FROM transactions WHERE transaction_type=1");
	$totalCommissionQuery->execute();
	$totalCommission = $totalCommissionQuery->fetch(PDO::FETCH_ASSOC);
	...
	$transactionQuery = $pdo->prepare("SELECT * FROM transactions
										INNER JOIN users ON transactions.user_id = users.id
										ORDER BY transaction_id DESC LIMIT {$offset}, {$no_of_records_per_page}");
				...
				<div class="col-xl-12">
					<div class="dashboard-box margin-top-0">
						<div class="notification success">
							<p><strong>Yığılan kommissiyaların ümumi məbləği: <?= $totalCommission['total_commission'];?> AZN</strong></p>
						</div>
						<!-- Headline -->
						<div class="headline">
								...
								<?php while($transaction = $transactionQuery->fetch(PDO::FETCH_ASSOC)){
									...
									<tr>
										...
										<td data-label="Əməliyyat ID">
											<?php if($transaction['transaction_type'] == 1){ ?>
												<a href="view_operation_admin.php?id=<?=$transaction['order_id'];?>"><?php echo $transaction['order_id'];?></a>
											<?php }?>
										</td>
										<td data-label="Frilanser"><?php echo $transaction['first_name']. ' '. $transaction['last_name'];?></td>
